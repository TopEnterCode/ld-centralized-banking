...
7. **Client flag scope.** Only `client-new-payment-ui` is made available to the browser SDK. Mock browser evaluation is performed by an isolated TypeScript provider using state supplied by the gateway. All payment validation and authority stay in Java.
